...
- `conversational_tokenizers_gain.ipynb`: Script implementing the retraining of the tokenizers. The original configurations of the tokenizers are preserved for all models, except for Mistral and Gemma. In these two cases, the default Hugging Face configuration is not designed to be retrained from scratch with a generic BPE trainer: it has no hard boundary between words, so retraining on it directly produces tokens that span multiple words. For these two models the pre-tokenizer is replaced with one that isolates word and punctuation into separate segments (so a merge can never cross a word boundary) while still keeping the real whitespace, glued to the following segment, instead of discarding it or replacing it with a generic marker — this gives an exact round-trip on decode, unlike the original Mistral/Gemma pre-tokenizer pipeline. Since both models declare `byte_fallback: true` but the plain retraining process does not populate the 256 byte-fallback tokens the fallback needs, those are added explicitly after training, keeping the total vocabulary size equal to the original model's (256 tokens are subtracted from, not added on top of, the merge budget). The retraining of the tokenizer can be performed in three different ways:
...
- `evaluation_loss_c4.ipynb`: Script that evaluates whether the retrained tokenizers introduce any performance degradation compared to the original one when applied to a subset of the [C4 AllenAI](https://huggingface.co/datasets/allenai/c4) dataset.  
  ...
  Results are provided in `loss_c4_retrained.csv`, where a positive value indicates that the retrained tokenizer performs better (produces fewer tokens) than the original one. This file is currently pending regeneration after the Mistral/Gemma tokenizer fix, so it is not included in the repo for now.

## Language-wise evaluation of conversation-optimized tokenizers
- `evaluation_gain_per_language.ipynb`: Studies the gain in efficiency of the new tokenizers for every language in the dataset. The results for the three retrained tokenizers (per model) are available in the folder [gains_by_language](./gains_by_language). Mistral and Gemma are currently pending regeneration after the tokenizer fix, so they are not included in the repo for now. This script also generates the file [`latin_conversations.txt`](./latin_conversations.txt), which contains a sample of 100 conversations labeled as Latin. Upon inspection, it becomes clear that this label is inaccurate.
...
- `evaluation_vocab.ipynb`: This notebook performs various analyses related to the vocabularies of different tokenizers. For Mistral and Gemma, whose vocabulary changed completely with the tokenizer fix, both results below are currently pending regeneration and not included in the repo for now:
